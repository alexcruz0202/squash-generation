...
def extract_squash_answers(next_key):
    ...
    while not os.path.exists('./squash_generation/squash/generated_outputs/inputs/%s/metadata.json' % next_key):
        print("Waiting for ./squash_generation/squash/generated_outputs/inputs/%s/metadata.json" % next_key)
        time.sleep(1.0)
    ...
    with open('./squash_generation/squash/generated_outputs/inputs/%s/input.pkl' % next_key, 'wb') as f:
